...
/*  @Test
  void shouldCreateTutorial() throws Exception {
    Tutorial tutorial = new Tutorial("om", "Spring Boot @WebMvcTest", "Description", true);
...
*/
  ...
  void shouldReturnListOfTutorials() throws Exception {
    List<Tutorial> tutorials = new ArrayList<>(
        Arrays.asList(new Tutorial("AA", "Spring Boot @WebMvcTest 1", "Description 1", true),
            new Tutorial("BB", "Spring Boot @WebMvcTest 2", "Description 2", true),
            new Tutorial("CC", "Spring Boot @WebMvcTest 3", "Description 3", true)));
...
/*  @Test
  void shouldReturnListOfTutorialsWithFilter() throws Exception {
    List<Tutorial> tutorials = new ArrayList<>(
        Arrays.asList(new Tutorial("A", "Spring Boot @WebMvcTest", "Description 1", true),
            new Tutorial("C", "Spring Boot Web MVC", "Description 3", true)));
  ...
  void shouldUpdateTutorial() throws Exception {
    ...
    Tutorial tutorial = new Tutorial("A", "Spring Boot @WebMvcTest", "Description", false);
    Tutorial updatedtutorial = new Tutorial("A", "Updated", "Updated", true);
  ...
  void shouldReturnNotFoundUpdateTutorial() throws Exception {
    ...
    when(tutorialRepository.findByName(name)).thenReturn(Optional.empty());
    when(tutorialRepository.save(any(Tutorial.class))).thenReturn(updatedtutorial);

    mockMvc.perform(put("/api/tutorials/{name}", name).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(updatedtutorial)))
        .andExpect(status().isNotFound())
        .andDo(print());
  }

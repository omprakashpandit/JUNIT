...
*/
  ...
  public void should_find_tutorial_by_id() {
    Tutorial tut1 = new Tutorial("om","Tut#1", "Desc#1", true);
    entityManager.persist(tut1);

    Tutorial tut2 = new Tutorial("prakash","Tut#2", "Desc#2", false);
    entityManager.persist(tut2);

  //  Tutorial foundTutorial = repository.findById(tut2.getId()).get();
    Tutorial foundTutorial = repository.findByName(tut2.getName());
